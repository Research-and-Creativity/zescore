...
const KioskScoreDosen: React.FC = () => {
  const {
    evaluator, selectedTeam, selectedCategory, remaining,
    setStep, setRemaining, backToTeamSelect,
  } = useKioskStore()
  ...
  if (!evaluator || !selectedTeam || !selectedCategory) return null

  const numScore  = rawInput === '' ? NaN : Number(rawInput)
  const isInteger = rawInput !== '' && /^\d+$/.test(rawInput)
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (/^\d{0,3}$/.test(val)) { setRawInput(val); setError(null) }
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const stillRemaining = remaining.filter(r => r !== selectedCategory)
  ...
  return (
    <div className="w-full max-w-lg">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-xl shadow-slate-200/50">
        ...
        <div className="flex justify-between text-xs font-bold mb-2 px-1" style={{ color: 'var(--text-secondary)' }}>
          <span>0</span>
        ...
        <div className="relative mb-2">
          <input
            type="text" inputMode="numeric" value={rawInput}
            onChange={handleInput} placeholder="0" maxLength={3} autoFocus
            className="w-full text-center font-black tracking-tight rounded-2xl bg-slate-50 border-2 text-slate-900 placeholder:text-slate-300 focus:outline-none focus:bg-white transition-all shadow-inner"
            style={{
              fontSize: 64, lineHeight: 1.1, paddingTop: 20, paddingBottom: 20,
              borderColor: validMsg ? '#DC2626' : isValid ? '#16a34a' : 'var(--card-border)',
            }}
          />
          <span className="absolute right-5 bottom-5 text-sm font-semibold" style={{ color: 'var(--text-secondary)' }}>/ 100</span>
        </div>
        ...
        <div className="grid grid-cols-5 gap-2 mb-5">
          {[60, 70, 75, 80, 85, 90, 95, 100].map(v => (
            <button key={v} onClick={() => { setRawInput(String(v)); setError(null) }}
        ...
        <button onClick={() => setConfirm(true)} disabled={!isValid || loading}
