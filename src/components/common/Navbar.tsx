...
const PAGE_TITLES: Record<string, { title: string; subtitle: string }> = {
    ...
    '/admin/users': { title: 'Mahasiswa & Dosen', subtitle: 'Kelola data penilai yang terdaftar' },
...
export default function Navbar() {
    ...
    return (
        <header className="flex items-center justify-between px-6 py-4 shrink-0"
            style={{ background: 'var(--card-bg)', borderBottom: '1px solid var(--card-border)', boxShadow: '0 1px 0 rgba(112,144,176,0.08)' }}>
            <div>
                <h1 className="text-display font-semibold text-lg" style={{ color: 'var(--text-primary)' }}>{info.title}</h1>
                {info.subtitle && <p className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>{info.subtitle}</p>}
            </div>
